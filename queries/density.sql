...
CREATE TABLE io_poh_hist_st4kdm_log(
    io_log_83 INTEGER,
    poh INTEGER,
    count INTEGER,
    PRIMARY KEY (io_log_83, poh)
);

INSERT INTO io_poh_hist_st4kdm_log (io_log_83, poh, count)
SELECT CAST(log(10, (lba_r + lba_w))*1000/12 AS INTEGER), poh/30*30, sum(1)
FROM raw_logs
WHERE (lba_r + lba_w) > 0 AND model='ST4000DM000'
GROUP BY CAST(log(10, (lba_r + lba_w))*1000/12 AS INTEGER), raw_logs.poh/30;
-- useful max(io) is 1e12 and max(poh) is ~30,000
